...
export function useSpotifyPlayer() {
  const { isAuthenticated, setTrack, getAccessToken, setPremium } = useSpotifyStore();
  ...
  useEffect(() => {
    ...
    const initPlayer = () => {
      ...
      // The SDK emits "not_premium" when the account is on the free tier.
      // Surface this as a banner instead of a silent 403 on every play call.
      player.addListener("not_premium", (err?: { message?: string }) => {
        setPremium(
          false,
          err?.message ?? "Free-tier account — Spotify Web Playback requires Premium.",
        );
      });
  ...
  }, [isAuthenticated, setTrack, getAccessToken, setPremium]);
